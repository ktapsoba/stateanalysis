...
public class NewConstantPropagation extends ForwardFlowAnalysis<Unit, Map<Local, Set<State>>>{
	...
	private NewVisitor newVisitor = new NewVisitor();
	private List<String> errors = new ArrayList<>();
	private Map<Unit, Map<Local, Set<State>>> stateByUnitIn;
	private Map<Unit, Map<Local, Set<State>>> stateByUnitOut;
	
	public NewConstantPropagation(DirectedGraph<Unit> graph, Map<Local, Set<State>> extremalValue, Chain<Local> localVariables, InterProceduralCFG cfg) {
		super(graph);
		this.graph = graph;
		this.unitGraph = (UnitGraph) graph;
		this.cfg = cfg;
		this.extremalValue = extremalValue == null? new HashMap<Local, Set<State>>(): extremalValue;
	...
	@Override
	protected void flowThrough(Map<Local, Set<State>> input, Unit unit, Map<Local, Set<State>> output) {
		...
		Stmt stmt = (Stmt) unit;
		try{
			newVisitor.visit(stmt, input, output, cfg, localVariables);
	...
	@Override
	protected void copy(Map<Local, Set<State>> input, Map<Local, Set<State>> output) {
	...
	@Override
	protected void merge(Map<Local, Set<State>> input1, Map<Local, Set<State>> input2, Map<Local, Set<State>> output) {
		copy(input1, output);
	...
	private void saveStateByUnit(Unit unit, Map<Local,Set<State>> state, Map<Unit, Map<Local,Set<State>>> unitByState){
